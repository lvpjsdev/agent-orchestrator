...
// .agents/config.json (optional)
{
  "continue": {
    ...
    "checkpoint_prefix": "[burlaki-checkpoint]"
  }
}
```
...
|-----------|----------------|--------|
| No brainstorm file | `start` | Run `/burlaki-start` |
| Brainstorm exists, no plan | `plan` | Run planning phase |
| Plan exists, no PRD | `prd` | Run PRD generation |
| PRD exists, stories pending | `run` | Resume `/burlaki-run` |
...
```bash
CHECKPOINT_MSG="[burlaki-checkpoint] Pre-continue snapshot
...
3. Restart from beginning
   - Start fresh with /burlaki-start
...
git commit -m "[burlaki-checkpoint] Drift fixes applied
...
|----------------|--------|
| Phase 1: Brainstorm | Run `/burlaki-start` (fresh) |
| Phase 2: Plan | Run planning from `/burlaki-start` |
| Phase 3: PRD | Generate PRD from existing plan |
| Next story | Run `/burlaki-run` from that story |
...
Recommendations:
1. Run /burlaki-continue to fix drift and resume
...
2. Create PR
3. Start new feature with /burlaki-start
```
...
Checkpoint commands:
├── Find: git log --grep='[burlaki-checkpoint]'
├── Restore: git reset --hard abc1234
└── Squash: git rebase -i abc1234~1

Handing off to /burlaki-run...
═══════════════════════════════════════════════════════════════
```
