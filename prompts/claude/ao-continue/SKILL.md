...
2) Run one or more iterations to resume:
...
3) Summarize result from latest run metadata/log:
...
4) If a story is stuck across attempts, propose escalation with `@human` tag and show it via `/ao-human`.
